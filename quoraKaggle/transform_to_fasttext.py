...
if __name__ == '__main__':
    train_data = 'fast_text.train'
    valid_data = 'fast_text.test'
    parse_train_to_ft_format('quora/train.csv', train_data, valid_data)
    ...
    model.save_model("quora.bin")
    model.quantize(input=train_data, qnorm=True, retrain=True, cutoff=100000)
    print_results(*model.test(valid_data))
    model.save_model("quora.ftz")

    print(model.test_label(valid_data))
    print(model.test('fast_text.test'))
